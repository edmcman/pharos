...
% XXX: Another ugly negation rule...
neg(reasonNOTMergeClasses_P(Class1Sorted, Class2Sorted)) :-
    factDeletingDestructor(Method),
    factRealDestructor(Caller)
...
neg(certainMemberOnExactClass(Class, Offset, Size)) :-
    certainMemberNOTOnExactClass(Class, Offset, Size).
